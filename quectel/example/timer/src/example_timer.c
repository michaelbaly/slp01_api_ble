...
{	
	g_sim_counter++;
	qt_uart_dbg(uart_conf.hdlr,"[timer callback]g_sim_counter is: %d", g_sim_counter);
	if(g_sim_counter % 2)
	{
...
int quectel_task_entry(void)
{
	...
	/* use txm timer directly */
	ret = tx_timer_create(&gpio_sim, "gpio simulate timer", gpio_driver_random, 10, \
						1000, 1000, TX_AUTO_ACTIVATE);
	if(ret)
	{
		qt_uart_dbg(uart_conf.hdlr,"[g_switch_timer] error code", ret);
	}
	...
	while (1)
	{
		...
		ret = relay_sta_get();
		qt_uart_dbg(uart_conf.hdlr,"[g_switch_timer] ignition status : %d", ret);
