#if defined(__EXAMPLE_SPRINTF__)
...
#define __my_va_rounded_size(t) ((sizeof(t) + sizeof(int) - 1)&~(sizeof(int) - 1))
#endif
...
static char* atel_fcvtbuf(double value, int ndigits, int* decpt, int* sign, char* buf)
{
	...
	*decpt = dot ? (dot - s) : atel_strlen(s);
	...
	else
	{
		...
		__atel_ecvround(buf, buf + *decpt + ndigits - 1, s + *decpt + ndigits + 1, decpt);
		pchRet = buf; /*return buf;*/
	}
	/*delete [] cvtbuf; */
	return pchRet;
