...
static int tcp_netctrl_init(void)
{
	...
	do
	{
		TCP_UART_DBG("Registering Callback udp_dss_handle\n");
		
		/* Obtain data service handle */
		status = qapi_DSS_Get_Data_Srvc_Hndl(tcp_net_event_cb, NULL, &udp_dss_handle);
		TCP_UART_DBG("tcp_dss_handle %d, status %d\n", udp_dss_handle, status);
		
		if (NULL != udp_dss_handle)
		{
			TCP_UART_DBG("Registed udp_dss_handler success\n");
			break;
		}
...
static int start_udp_session(void)
{
	...
	/* initialize a socket */
	qapi_fd_zero(&fd_list);

	/* create socket and add to the list */
	...
	do
	{
		...
		strcpy(buff, "Hello Atel, This is an UDP client Demo!");
		...
		while (1)
		{
			...
			recv_len = qapi_recvfrom(sock_fd, buff, SENT_BUF_SIZE, 0, (struct sockaddr *)&to, &tolen);
			if (recv_len > 0)
			{
...
int quectel_task_entry(void)
{
	...
	/* the demo is start */
	TCP_UART_DBG("Atel UDP Client Demo is start!");
    ...
    tx_byte_pool_create(byte_pool_udp, "tcp application pool", free_memory_udp, UDP_BYTE_POOL_SIZE);
